...
k_DX=kernel.f(D,x);
 ...
 %compute current posterior distribution parameters
    ...
    Sigma_full=k_XX-k_DX'/(KDD + noise_prior^2*eye(size(KDD)))*k_DX ; 
...
%compute mean and covariance with index Zi removed
   ...
   Sigma_removal=k_XX-k_DX(remains,:)'/(KDD(remains,remains) ...
               + noise_prior^2*eye(size(KDD(remains,remains))))*k_DX(remains,:); 
      
   

      % compute error for this removal set, see if smallest
      gi =hellinger_distance(mu_removal,mu_full,Sigma_removal,Sigma_full);
      if gi<gmin
          gmin = gi;
          imin = i;
      end
...
%compute mean and covariance with index Zi removed
   mu_removal_return=k_DX(remains,:)'/(KDD(remains,remains) ...
            + noise_prior^2*eye(size(KDD(remains,remains))))*y(remains)'; 
   Sigma_removal_return=k_XX-k_DX(remains,:)'/(KDD(remains,remains) ...
               + noise_prior^2*eye(size(KDD(remains,remains))))*k_DX(remains,:); 
