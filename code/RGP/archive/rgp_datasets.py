...
def CreateBasisVectors(X_values, input_size, version="CEL", num_intervals=3):    
    ...
    if(version=="CEL"):        
        ...
        list_base_coordinates=[ np.linspace(lb[i], ub[i], num_intervals)  for i in range(input_size)]
        for ls in list_base_coordinates:
            np.random.shuffle(ls)
        return(np.vstack(tuple(list_base_coordinates)).T, lb,ub)

def plotting(evaluation, label, scalar=1000, nbins=100):
...
def testrun(num_intervals=530, ls=4.17, iterations=100, version="JB", description=False, FULLGP=False, PLOTTING=False):
    ...
    dataframe = np.loadtxt("datasets/dataset6.csv",delimiter=","); input_size=9; index_sep = 9; output_size=6; 
    ...
    X_base,lb,ub = CreateBasisVectors(X_values, input_size,version=version,num_intervals=num_intervals)
    ...
    for i in range(output_size):
        model_aux = recursiveGP(variance = 1.0, lengthscales = ls, sigma = 0.01)
...
#    for j in range(iterations):        
    ...
    difference  = comparePrediction([modelsRGP[i].predict for i in range(output_size)] , XTest, np.transpose(YTest))
...
#rint(testrun(530,ls=4.17,iterations=80))
