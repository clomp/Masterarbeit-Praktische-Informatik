...
#dataset="dataset2" ; input_size=12; limitation=3000;
output_size=3
scalar=1000
xlims=1.0
print("#### "+dataset+" ####")
...
for i in range(output_size):
    ...
    if(limitation is None):
        ...
        YTrn = YTrain[:limitation,i].reshape((limitation,1))
        model_aux=gpflow.models.GPR(data=(XTrain[:limitation], YTrn), 
                      kernel=gpflow.kernels.SquaredExponential(variance=1.0,lengthscales=LS), mean_function=None)
               
        
    model_aux.likelihood.variance.assign(0.01)
    opt = gpflow.optimizers.Scipy()
    opt_logs = opt.minimize(model_aux.training_loss, model_aux.trainable_variables)        
    Lengthscales.append(list(model_aux.kernel.lengthscales.numpy()))
    varianz=model_aux.kernel.variance.variables[0].value().numpy()
    print(model_aux.likelihood.variance)
    if(varianz < 0):
    ...
    YTest_prediction.append(YTst_aux.numpy())
