...
from GPEnKFmaster.gpenkf.gp_util.covariance_function import CovarianceFunction
...
class SquaredExponential(CovarianceFunction):
    ...
    def eval_cov_matrix(self, x1, x2):
        return self.variance * np.exp(-cdist(x1, x2)**2/(2 * self.lengthscale**2))

    def predict(self, x_new, x_g, noise_variance, g):
        # TODO estimate lengthscale for each dimension        
        
        #K_new_g = self.variance * np.exp(-cdist(x_new, x_g, 'seuclidean', V=2 * self.lengthscale **2 *np.eye(6) )** 2)
        K_new_g = self.eval_cov_matrix(x_new , x_g)

        #K_g_g = self.variance * np.exp(-cdist(x_g, x_g, 'seuclidean', V=2 * self.lengthscale **2 * np.eye(6) )** 2)
        K_g_g = self.eval_cov_matrix(x_g, x_g)

        #K_new_new = self.variance * np.exp(-cdist(x_new, x_new, 'seuclidean', V=2 * self.lengthscale **2 * np.eye(6))** 2)
        K_new_new = self.eval_cov_matrix(x_new, x_new)
    ...
    def log_likelihood(self, y, x_g, noise_variance):
        ...
        #K_g_g = self.variance * np.exp(-cdist(x_g, x_g, 'seuclidean', V=2 * a **2 * np.eye(6))** 2)
        K_g_g = self.variance * np.exp(-cdist(x_g, x_g)**2/ ( 2 * a**2))
